...
Status: **proposed.** Stage 0 (§5) has been run; nothing else has.
...
- $c$ — a scalar model-complexity index (§6),
...
at an oracle budget and selection at a threshold** (§9, R3).
...
- **H3 Estimand-limited ceiling** — $g$ plateaus low for reasons unrelated to
  $c$. **Confirmed for `conditional_interaction`, see §5.**
...
## 4. ⚠ Two blockers, both verified on `main` @ `04762e3`

### W0 — the repo does not import

`lfs/data/__init__.py` and `lfs/data/patches.py` are **absent from the working
tree and from git**, excluded by the unanchored `data/` rule at `.gitignore:42`
(intended for the 63 MB MNIST dump at `/data/`, but it also matches `lfs/data/`):

```
$ git check-ignore -v lfs/data/__init__.py
.gitignore:42:data/    lfs/data/__init__.py

$ python -c "import experiments.run"
ImportError: cannot import name 'get_patches' from 'lfs.data' (unknown location)

$ pytest -q
ERROR tests/test_patches.py · tests/test_runner.py · tests/test_selection.py
3 errors in 0.80s
```

`README.md` advertises 103 passing tests; the suite currently cannot collect.
Fix: anchor the rule to `/data/` and commit the two files. Both exist on
`origin/maintenance-pass-2026-08-21` (commit `6d8870b`) and can be recovered
with `git checkout origin/maintenance-pass-2026-08-21 -- lfs/data/`.
**Nothing below can run until this is done.**

### W1 — the width sweep would be a silent no-op

`lfs/models/nn.py:101`, `_AdaptiveMixin.initialize_module`:
...
`hidden` and `depth` are never passed through. Every arm of a complexity sweep
would train the identical default `DeepMLP(hidden=256, depth=3)`, and $g(c)$
would come out flat for an entirely uninteresting reason — a flat curve that
*looks* like H1 with a low plateau. This is the one bug that would make the
whole experiment quietly meaningless. Fix before Stage 1, with a regression test
asserting parameter count actually changes with `hidden`.

Environment is otherwise fine: `/opt/anaconda3/bin/python` has torch 2.11.0,
captum 0.9.0, skorch 1.3.1, sklearn 1.5.1.

---

## 5. Stage 0 — RUN, and it changes the design
...
- **The main sweep moves to a smooth-gate design** (W5): replace
...
- **Break ties randomly in `select_top_k`.** The current `np.argsort` is
  ...
  Any design with exact-zero attributions inherits this bias (W8).

---

## 6. The complexity axis
...
## 7. Data and query points

Main: `smooth_conditional_interaction(tau=0.1)` (W5). Control:
...
generalises** until a variable-sparsity design exists (W6). Treat it as a
...
| C1 | analytic gradient of true $f$ | ceiling of saliency-as-estimand — **done, §5** |
| C2 | linear regression, $a_j = \|\beta_j\|$ | zero-complexity anchor; constant in $\mathbf{x}$, so local F1 = global F1 |
...
| ID | Rule | Isolates |
|---|---|---|
| S-oracle | top-$k$, $k=\|S^*_{\mathbf{x}}\|$ (`select_top_k`, random tie-break) | ranking quality alone |
...
- **R2 — Coverage / Localization** (§10): the precision/recall decomposition of
...
- **R5 — F1 by feature role** (gates vs. active pair) and **stratified by gate
  margin** $|x_3|$. Guards §5; on the smooth design, gate recall must decrease
...
3. **Grid vs. point.** They aggregate over channels × time; we score one
   ...
   of variation is the query point (§11).

Not in `notes/ref.bib` — add before citing (W7).

---

## 11. Stage 1, stop rule, error bars
...
(§5), so always show the distribution (R7), never only the mean.
...
| # | Work | Why |
|---|---|---|
| **W0** | Anchor `.gitignore:42` to `/data/`; restore `lfs/data/__init__.py` + `patches.py` | **Blocking.** Repo does not import; 3 test modules fail collection (§4). |
| **W1** | Plumb `hidden`/`depth` through `_AdaptiveMixin.initialize_module` (skorch `module__*` kwargs) | **Blocking.** Otherwise every arm trains the same net (§4). |
| **W2** | Add `kind: saliency` to `experiments/run.py` — build, fit, attribute over a *batch* of query points | Runner dispatches only `minshap`/`max_p`/`loco`. |
| **W3** | Batch local scoring: `local.query_index` is a single int and sweeping it writes one result dir per point. Add `local.query_indices` with one `metrics.json` of per-point rows | 200 dirs per arm is unusable. |
| **W4** | Store raw attributions in `arrays.npz` ($M \times p$) so the $\tau$ grid, S-tang, S-oracle, R4 and R7 recompute offline from one run | Makes the rule-crossing genuinely free and re-analysable without refitting. |
| **W5** | `smooth_conditional_interaction(tau=...)` in `lfs/data/simulated.py` with matching `local_support` | §5. The main design. |
| **W6** | A variable-$\|S^*_{\mathbf{x}}\|$ design | §7. Needed before any threshold claim generalises. |
| **W7** | Promote `scratchpad/stage0.py` into `experiments/`; add Tang et al. to `notes/ref.bib` | §5 is now a citable result and needs a seed + SHA behind it. |
| **W8** | Random tie-breaking in `select_top_k`; tests for quadrant balance, F1 of a known mask, and that W1 changes param count | §5 — deterministic argsort manufactures fake recall on exact-zero attributions. |
